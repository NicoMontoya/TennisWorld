 ...
 * Mock Data Generator
 * 
 * This file provides functions to generate mock data for the tennis-api MCP server.
 * It simulates the data that would be returned by the real MCP server.
 */

/**
 * Generate "live" rankings data with some randomization
 * @param {string} type - The type of rankings to generate (ATP or WTA)
 * @returns {Array} - An array of player rankings
 */
export const generateLiveRankings = (type) => {
  const baseRankings = type.toUpperCase() === 'WTA' ? getBaseWTARankings() : getBaseATPRankings();
  
  // Add randomization to make it feel "live" and changes more noticeable
...
/**
 * Generate "live" tournament data with some randomization
 * @returns {Array} - An array of tournaments
 */
export const generateLiveTournaments = () => {
